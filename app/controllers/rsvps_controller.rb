class RsvpsController < ApplicationController
  ...
  def create
    ...
    if @rsvp.save
      ...
      send_sms(recipient: current_user, message: "You're hanging out with #{@event.host.first_name} today at #{@event.location.name} at #{@rsvp.arrival_time_name}. #{@event.host.first_name} will be wearing a #{@event.host_outfit}!")
      send_sms(recipient: @event.host, message: "You've got a hang! #{current_user.first_name} is wearing a #{@rsvp.outfit}, and will be at #{@event.location.name} at #{@rsvp.arrival_time_name}")
    else
  ...
  def destroy
    ...
    if @rsvp.creator == true
      ...
      @event.users.each do |person|
        send_sms(recipient: person, message: @cancellation_message) unless person == current_user
      end
    else
      # If a guest cancels, only text the host, not everyone
      send_sms(recipient: @event.host, message: @cancellation_message)
    end
